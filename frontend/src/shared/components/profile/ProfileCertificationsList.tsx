...
export function ProfileCertificationsList({
  ...
  return (
    <section className="relative rounded-2xl border border-maps-border bg-white p-6 shadow-profile sm:p-8">
      ...
      {certificaciones.length > 0 ? (
        <ul className="mt-4 flex flex-col gap-3">
          {certificaciones.map((cert) => (
            <li
              key={cert.id}
              className="flex items-center justify-between gap-3 rounded-2xl border border-maps-border bg-slate-50 p-3.5"
            >
