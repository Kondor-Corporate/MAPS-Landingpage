...
- **Mock + Zustand en memoria**, mismo patrón de **store local** que MAPS-007 usaba en **productores** antes de MAPS-009; en noticias se manti hasta integrar backend. Cuando llegue API, se reemplaza el store sin tocar idealmente la UI.
...
- **IDs generados con `Date.now()` + random** en cliente (análogo a otros mocks admin). Razón: sin backend persistente para noticias. Cuando llegue backend, IDs server-side.
